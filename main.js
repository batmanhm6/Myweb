document.addEventListener('DOMContentLoaded', () => {
    // ---------------------------------------------------------
    // 0. API Initialization (Supabase & EmailJS)
    // ---------------------------------------------------------
    // IMPORTANT: Replace these placeholders with your actual keys
    const SUPABASE_URL = 'https://xuobtxzjhsiizuhtdvsd.supabase.co';
    const SUPABASE_ANON_KEY = 'sb_publishable_7SxgZ4tQDIGt30UVbICGCw_4OipbB0i';
    ...
    const fetchPosts = async () => {
        ...
        // 2. If Supabase is connected, try to get server data
        if (supabaseClient) {
            try {
    ...
    const renderPosts = () => {
        ...
        if (currentBoardSort === 'likes') {
            sortedPosts = [...posts].sort((a, b) => (b.likes - a.likes) || (b.id - a.id));
        } else {
    ...
    window.likePost = async (postId) => {
        ...
        if (supabaseClient) {
            supabaseClient.from('posts').update({ likes: newLikes }).eq('id', postId)
        ...
        boardForm.addEventListener('submit', async (e) => {
            ...
            if (author && password && content) {
                ...
                if (supabaseClient) {
                    try {
                        if (editId) {
                            await supabaseClient.from('posts').update({ author, password, content }).eq('id', editId);
                        } else {
                            // Convert for DB (id handled by DB)
                            const dbPost = { ...newPost };
                            delete dbPost.id; 
                            await supabaseClient.from('posts').insert([dbPost]);
                        }
                    } catch (e) {
            ...
            if (inputPass === targetPost.password) {
                if (currentAction === 'delete') {
                    if (confirm('정말 삭제하시겠습니까?')) {
                        ...
                        // 2. Sync with Supabase in background
                        if (supabaseClient) {
                            supabaseClient.from('posts').delete().eq('id', currentTargetId)
