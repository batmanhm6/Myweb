document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof firebase !== 'undefined') {
        try {
            ...
            alert('Firebase 초기화에 실패했습니다. API 키나 설정을 확인해주세요.\n오류: ' + e.message);
        }
    } else {
        console.warn('Firebase SDK is not loaded. Check your internet connection or script tags.');
        // Don't alert here yet, as it might be annoying on every page load if deliberate
    }
    ...
    const fetchPosts = async () => {
        ...
        if (db) {
            db.collection('posts').orderBy('created_at', 'desc').onSnapshot((snapshot) => {
                ...
                // Firestore 데이터를 전역 posts 변수에 할당하고 렌더링
                posts = data;
                renderPosts();
            }, (e) => {
                console.error('Firebase sync failed:', e.message);
                if (e.message.includes('permission')) {
                    alert('데이터를 불러올 권한이 없습니다. Firebase Console에서 보안 규칙(Rules)을 확인해주세요.');
                }
            });
        } else {
            console.error('Database connection not established. Make sure Firebase is initialized.');
        }
    };
    ...
    const renderPosts = () => {
        ...
        sortedPosts.forEach(p => {
            if (p.parent_id && postMap[p.parent_id]) {
                ...
                // 부모가 없거나(루트 게시글), 부모가 존재하지 않는 답글은 루트에 표시하지 않음
    ...
    window.likePost = async (postId) => {
        ...
        // UI 즉시 업데이트를 위해 메모리 내 posts 업데이트
        post.likes = newLikes;
        renderPosts();
    ...
    if (boardForm) {
        boardForm.addEventListener('submit', async (e) => {
            ...
            if (!db) {
                alert('데이터베이스 연결이 활성화되지 않았습니다. 인터넷 연결이나 Firebase 설정을 확인해주세요.');
                return;
            }

            if (author && password && content) {
                // 임시 객체 생성 (ID는 나중에 Firebase에서 결정되거나 수정 시 유지)
                const newPost = {
                ...
                // 1. Optimistic UI: 화면에 즉시 표시하기 위해 임시 ID 부여
                const tempId = editId ? editId : 'temp_' + Date.now();
                const optimisticPost = { id: tempId, ...newPost };

                if (editId) {
                    const idx = posts.findIndex(p => String(p.id) === String(editId));
                    if (idx !== -1) posts[idx] = optimisticPost;
                } else {
                    posts.unshift(optimisticPost);
                }
                
                renderPosts();
                const originalSubmitText = boardSubmitBtn.textContent;
                boardSubmitBtn.textContent = '저장 중...';
                ...
                try {
                    if (editId) {
                        ...
                        console.log('Post updated in Firebase:', editId);
                    } else {
                    ...
                    alert('데이터를 저장하지 못했습니다.\n\n원인: ' + e.message + '\n\nFirebase Console의 Firestore Rules 탭에서 쓰기 권한이 허용되어 있는지 확인해 보세요.');
                    
                    // 실패 시 로컬 목록에서 제거 (새로고침 시와 동일한 상태로 복구)
                    if (!editId) {
                        posts = posts.filter(p => p.id !== tempId);
                        renderPosts();
                    }
                } finally {
                    boardSubmitBtn.textContent = originalSubmitText;
            ...
            if (inputPass === targetPost.password) {
                if (currentAction === 'delete') {
                    if (confirm('정말 삭제하시겠습니까?')) {
                        ...
                        // 로컬 배열 임의 수정(splice) 제거 - Firebase 리스너(onSnapshot)가 자동으로 화면을 갱신하도록 변경
                        closeBoardModal();

                        // 3. Firebase 서버에 삭제 요청
                        if (db) {
                            console.log('Firebase 삭제 시도 중... ID:', currentTargetId);
                            try {
                                const deletePromises = [];
                                // 본문 삭제
                                deletePromises.push(db.collection('posts').doc(String(currentTargetId)).delete());
                                
                                // 답글 삭제
                                const childPostsToDelete = posts.filter(p => String(p.parent_id) === String(currentTargetId));
                                childPostsToDelete.forEach(child => {
                                    deletePromises.push(db.collection('posts').doc(String(child.id)).delete());
                                });

                                // 모든 삭제 요청이 끝날 때까지 기다립니다.
                                await Promise.all(deletePromises);
                                alert('서버 삭제 완료\\n삭제된 ID: ' + currentTargetId);
                            } catch (e) {
                                console.error('Firebase 삭제 에러:', e);
                                alert('서버 삭제에 실패했습니다.\\n오류: ' + e.message);
                            }
                        } else {
                            alert('오류: 데이터베이스(db) 연결이 끊겨있습니다. 페이지를 새로고침해 주세요.');
                        }
                    }
